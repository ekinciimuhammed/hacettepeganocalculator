...
function normalizeAppUrl(url) {
    ...
    if (url === '/') return isToolsPage ? '../index.html' : './index.html';
    if (url.startsWith('/#')) return isToolsPage ? `../index.html${url.slice(1)}` : `./index.html${url.slice(1)}`;
    if (url.startsWith('/')) {
        const target = url.slice(1);
        if (target.endsWith('.html')) {
            return isToolsPage ? `./${target}` : `./tools/${target}`;
        }
        return isToolsPage ? `../${target}` : `./${target}`;
    }
    return url;
}
            ...
            <div class="container">
                ...
                <div class="related-tools-grid">
                    ${relatedTools.map(tool => `
                        <a href="${normalizeAppUrl(tool.url)}" class="related-tool-card">
