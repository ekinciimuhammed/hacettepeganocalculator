...
function toRelativeAppPath(href) {
    if (!href) return href;
    if (
    ...
    if (href === '/') return isToolsPage ? '../index.html' : './index.html';
    if (href.startsWith('/#')) {
        return isToolsPage ? `../index.html${href.slice(1)}` : `./index.html${href.slice(1)}`;
    }

    if (href.startsWith('/')) {
        const target = href.slice(1);
        if (isToolsPage) {
            if (target.startsWith('tools/')) return `./${target.slice('tools/'.length)}`;
            return `../${target}`;
        }
        return `./${target}`;
    }
...
function normalizeInternalLinks() {
    document.querySelectorAll('a[href]').forEach((anchor) => {
        ...
        const normalized = toRelativeAppPath(rawHref);
...
function applyCustomBranding() {
    // Intentionally disabled: branding/logos are removed in tools-only mode.
}
...
function enforceToolsOnlyLayout() {
    const isToolsPage = /\/tools\//.test(window.location.pathname);
    const mustRemoveSelectors = [
        '.hero',
        '#about',
        '#comparison',
        '#faq',
    ...
    mustRemoveSelectors.forEach((selector) => {
    ...
    if (nav) {
        ...
        let toolsLink = links.find((link) => {
            const href = (link.getAttribute('href') || '').toLowerCase();
            const text = (link.textContent || '').toLowerCase();
            return text.includes('tools') || href.includes('#tools');
        });

        if (!toolsLink) {
            toolsLink = document.createElement('a');
            toolsLink.className = 'nav-link';
            toolsLink.href = isToolsPage ? '../index.html#tools' : './index.html#tools';
    ...
    document.querySelectorAll('.logo, .logo-link, .footer-logo').forEach((el) => el.remove());

    document.querySelectorAll('.hamburger-menu, .mobile-nav, .mobile-overlay').forEach((el) => el.remove());
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Mobile navigation is intentionally disabled in tools-only mode.
