    ...
    async convertSvgToPng() {
        try {
            ...
            for (const file of this.uploadedFiles) {
                ...
                try {
                    const img = await new Promise((res, rej)=>{ const i=new Image(); i.onload=()=>res(i); i.onerror=()=>rej(new Error('SVG yüklenemedi')); i.src=url; });
            ...
            return results;
        } catch (e) { console.error('Error converting SVG to PNG:', e); throw new Error("SVG, PNG'ye dönüştürülemedi"); }
    },
    ...
    async svgFileToPngBytes(file) {
        ...
        try {
            const img = await new Promise((resolve, reject) => {
                const i = new Image();
                i.onload = () => resolve(i);
                i.onerror = () => reject(new Error('SVG yüklenemedi'));
                i.src = url;
            });
