document.addEventListener('DOMContentLoaded', () => {
    ...
    const gradeScalePresets = {
        ...
        aabb: {
            'AA': 4.00, 'AB': 3.70, 'BA': 3.50,
            'BB': 3.00, 'BC': 2.70, 'CB': 2.50,
            'CC': 2.00, 'CD': 1.70, 'DC': 1.50,
    ...
    // Initialize gradeScale with Hacettepe default (fallback for any unset state)
    ...
    async function handleFile(file) {
        ...
        try {
            ...
            // 3. Fallback: Custom Parser (app's own PDF export format)
            ...
            // 4. Fallback: Hacettepe Official Transcript Parser
            if (!extractedCourses || extractedCourses.length === 0) {
                console.log("Custom parser found no courses. Attempting Hacettepe official transcript parser...");
                if (typeof extractHacettepeOfficial === 'function') {
                    extractedCourses = extractHacettepeOfficial(pages);
                } else {
                    console.error("Hacettepe Official Parser not loaded!");
                }
            }
            ...
            // Supplement with department info detected by Hacettepe Official Parser if needed
            if (typeof extractHacettepeOfficial === 'function' && extractHacettepeOfficial._lastInfo) {
                const h = extractHacettepeOfficial._lastInfo;
                if (!headerInfo.academicUnit && h.faculty) headerInfo.academicUnit = h.faculty;
                if (!headerInfo.program && h.department) headerInfo.program = h.department;
                extractHacettepeOfficial._lastInfo = null; // reset for next upload
            }
            if (headerInfo.mode) {
    ...
    function extractGradeScale(pages) {
        ...
        // Filter out non-grade entries (e.g. K=Kredi, I/II=Roman numerals for semesters)
        for (const key of Object.keys(scale)) {
            if (!/^[ABCDF]/i.test(key)) delete scale[key];
        }
    ...
    function calculateSmartTarget(targetGno, userTargetGradeVal, isRealistic) {
        ...
        targetResultArea.innerHTML = `
            ...
            <div>Tüm dersleri ${userTargetGradeVal.toFixed(2)} ile tekrar etseniz bile maksimum <strong>${maxPossibleGno.toFixed(2)}</strong> ortalamaya ulaşabilirsiniz.</div>
        `;
    }
    ...
    const pdfToolsIframe = document.getElementById('pdf-tools-iframe');
    let gradeChart = null;
    let trendChart = null;
    ...
    tabButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (targetView) {
                ...
                if (targetId === 'view-useful-tools' && pdfToolsIframe && !pdfToolsIframe.src) {
                    const src = pdfToolsIframe.getAttribute('data-src');
                    if (src) pdfToolsIframe.src = src;
                }
            }
        });
    });
    ...
    window.openGradeScaleEditor = function () {
        gsEditorTemp = { ...gradeScale };
        document.getElementById('gs-preset-select').value = '';
        renderGsEditorEntries();
        document.getElementById('grade-scale-editor-modal').classList.remove('hidden');
    };

    window.closeGradeScaleEditor = function () {
        document.getElementById('grade-scale-editor-modal').classList.add('hidden');
    };

    window.applyGradeScalePreset = function (key) {
        if (!key || !gradeScalePresets[key]) return;
        gsEditorTemp = { ...gradeScalePresets[key] };
        renderGsEditorEntries();
    };

    window.addGradeEntry = function () {
        const container = document.getElementById('gs-editor-entries');
        const row = document.createElement('div');
        row.className = 'gs-editor-row';
        row.innerHTML = `
            <input type="text" class="gs-grade-input" placeholder="Harf (ör: A1)" maxlength="4">
            <input type="number" class="gs-point-input" placeholder="0.00" step="0.25" min="0" max="4.00">
            <button class="gs-remove-btn" onclick="this.closest('.gs-editor-row').remove()">
                <i class="fa-solid fa-trash"></i>
            </button>`;
        container.appendChild(row);
        row.querySelector('.gs-grade-input').focus();
    };

    window.saveGradeScaleEditor = function () {
        const rows = document.querySelectorAll('#gs-editor-entries .gs-editor-row');
        const newScale = {};
        let hasError = false;
        rows.forEach(row => {
            const grade = row.querySelector('.gs-grade-input').value.trim().toUpperCase();
            const val = row.querySelector('.gs-point-input').value;
            const point = parseFloat(val);
            if (!grade && !val) return; // boş satır, atla
            if (!grade || isNaN(point) || point < 0 || point > 4) {
                hasError = true;
                return;
            }
            newScale[grade] = point;
        });
        if (hasError) { alert('Bazı satırlarda eksik veya geçersiz değer var (0–4 arası olmalı).'); return; }
        if (Object.keys(newScale).length === 0) { alert('En az bir not girişi olmalı.'); return; }
        gradeScale = newScale;
        closeGradeScaleEditor();
        renderGradeScale();
        renderCourses();
        calculateStats();
        saveToLocalStorage();
    ...
    document.getElementById('grade-scale-editor-modal').addEventListener('click', function (e) {
        if (e.target === this) closeGradeScaleEditor();
    });

    function renderGsEditorEntries() {
        const container = document.getElementById('gs-editor-entries');
        container.innerHTML = '';
        const sorted = Object.entries(gsEditorTemp).sort(([, a], [, b]) => b - a);
        sorted.forEach(([grade, point]) => {
            const row = document.createElement('div');
            row.className = 'gs-editor-row';
            row.innerHTML = `
                <input type="text" class="gs-grade-input" value="${grade}" maxlength="4">
                <input type="number" class="gs-point-input" value="${point.toFixed(2)}" step="0.25" min="0" max="4.00">
                <button class="gs-remove-btn" onclick="this.closest('.gs-editor-row').remove()">
                    <i class="fa-solid fa-trash"></i>
                </button>`;
            container.appendChild(row);
